...
export function TemplateGenerator({ leads, useCases, tones, products }: Props) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={generate} className="rounded bg-slate-900 px-3 py-2 text-sm text-white" disabled={loading}>{loading ? "Generating..." : "Generate"}</button>
      <div>
      ...
      <div className="flex gap-2">
        <button type="button" onClick={() => save("template")} className="rounded border px-3 py-2 text-sm">Save as Template</button>
        <button type="button" onClick={() => save("draft")} className="rounded border px-3 py-2 text-sm">Save as Communication Draft</button>
      </div>
